...
var externalRequire = getCustomRequire();
var { NFC } = externalRequire("nfc-pcsc");
var notifier = externalRequire("node-notifier");
var bindings = externalRequire("bindings");
var SUPABASE_URL = "https://zdirmkypfxuamjbdkwhb.supabase.co";
var SUPABASE_KEY = "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6InpkaXJta3lwZnh1YW1qYmRrd2hiIiwicm9sZSI6InNlcnZpY2Vfcm9sZSIsImlhdCI6MTc2NjE1MzgxNCwiZXhwIjoyMDgxNzI5ODE0fQ.CORI1-tLzRPgdqVYxY_HX6eGDasc0l8s9muSS-eGIuk";
var YAMEN_SECRET = "e18105f3642bcb546d0790eafef801219c9602915e42413a1e62fffa43434100";
...
async function loadTerminalConfig() {
  ...
  let terminalId2 = 1;
  let terminalName2 = "New Scanner";
  try {
    const idInput = await askQuestion("Enter Terminal ID (default: 1): ");
    if (idInput.trim()) terminalId2 = parseInt(idInput.trim(), 10) || 1;
    const nameInput = await askQuestion("Enter Terminal Name (default: New Scanner): ");
    if (nameInput.trim()) terminalName2 = nameInput.trim();
    const config = { terminalId: terminalId2, terminalName: terminalName2 };
    const newConfigPath = configPaths[0];
...
async function bootstrap() {
  console.log("\n\u{1F4C2} Loading configuration files...\n");
  ...
  function generateSignature(uid) {
    const hmac = crypto2.createHmac("sha256", YAMEN_SECRET);
    hmac.update(uid);
  ...
  async function ensureTerminalExists(terminalId2, terminalName2) {
    try {
      const { data: existing, error: checkError } = await supabase.from("terminals").select("id").eq("id", terminalId2).maybeSingle();
      if (existing) {
        console.log(`\u2705 Terminal ${terminalId2} exists in database`);
        return true;
      }
      const { data: branches, error: branchError } = await supabase.from("branches").select("id").limit(1).maybeSingle();
      let branchId = 1;
      if (branches && branches.id) {
        branchId = branches.id;
      } else {
        const { data: newBranch, error: createBranchError } = await supabase.from("branches").insert([{
          name: "Default Branch",
          location: "Main"
        }]).select("id").single();
        if (newBranch) {
      ...
      console.log(`\u26A0\uFE0F  Terminal ${terminalId2} not found. Creating...`);
      const { data: newTerminal, error: createError } = await supabase.from("terminals").insert([{
        id: terminalId2,
        branch_id: branchId,
        name: terminalName2,
        connection_url: "local://nfc-bridge",
        terminal_secret: crypto2.randomUUID()
      }]).select().single();
      if (createError) {
        console.error(`\u274C Failed to create terminal: ${createError.message}`);
        return false;
      }
      console.log(`\u2705 Terminal created successfully: ID ${terminalId2}`);
      return true;
    } catch (err) {
  ...
  const deviceTracking = /* @__PURE__ */ new Map();
  let readerCounter = 0;
  ...
  function recordScan(deviceId, uid, isSecured) {
    ...
    if (device) {
      device.scansCount++;
      device.lastScan = (/* @__PURE__ */ new Date()).toISOString();
    }
  }
  ...
  const DEBOUNCE_SAME_CARD_MS = 800;
  const DEBOUNCE_NEW_CARD_MS = 50;
  const readerState = /* @__PURE__ */ new Map();
  function shouldProcessScan(deviceId, uid) {
    ...
    console.log(`\u{1F504} [SWITCH] Switching from ${state.uid} to ${uid}`);
    state.uid = uid;
  ...
  async function syncWithRetry(payload, maxRetries = 3) {
    for (let attempt = 1; attempt <= maxRetries; attempt++) {
      try {
        const { error, data } = await supabase.from("scan_events").insert([payload]).select().single();
        if (!error) {
  ...
  async function updateEventWithRetry(eventId, updates, maxRetries = 2) {
    for (let attempt = 1; attempt <= maxRetries; attempt++) {
      try {
        const { error } = await supabase.from("scan_events").update(updates).eq("id", eventId);
  ...
  setInterval(() => sendHeartbeat(false), 1e4);
  sendHeartbeat(false);
  const nfc = new NFC();
  console.log("================================================");
  console.log("   \u{1F680} NFC CLOUD BRIDGE (STANDALONE v2.0)");
  console.log("================================================");
  console.log(`\u{1F4E1} Status: Active | Ready for cloud sync`);
  console.log(`\u{1F3EA} Terminal: ${terminalName} (ID: ${terminalId})`);
  console.log("================================================\n");
  ...
  nfc.on("reader", (reader) => {
    ...
    activeReaders.set(deviceId, reader);
    console.log(`
[READER] Device found: ${reader.name} (${deviceId})`);
    ...
    reader.on("card", async (card) => {
      console.log(`
\u26A1 [INSTANT] Card detected at: ${(/* @__PURE__ */ new Date()).toISOString()}`);
      const uid = card.uid.toUpperCase();
      if (!shouldProcessScan(deviceId, uid)) {
        return;
      }
      console.log(`
\u{1F3B4} [SCAN] New card: ${uid}`);
      try {
        ...
        let readDebugData = null;
        try {
          const performVerification = async () => {
            try {
              await reader.authenticate(4, 96, "FFFFFFFFFFFF");
          ...
          const timeoutProfile = new Promise((resolve) => setTimeout(() => resolve(false), 800));
        ...
        if (!isValidYamen) {
          ...
          }).eq("uid", uid).is("metadata->secured", null).then(({ error }) => {
...
Type: ${cardType}`,
          ...
          console.log(`\u{1F504} [READY] Waiting for card removal...
`);
        }
      } catch (err) {
    ...
    reader.on("card.off", async (card) => {
      ...
      if (postAwaitState && postAwaitState.uid === removedUid) {
        clearReaderState(deviceId);
        console.log(`\u{1F504} System ready for new card
`);
      }
    });
    reader.on("error", (err) => {
    ...
    reader.on("end", () => {
      console.log(`\u{1F50C} [READER DISCONNECTED] ${reader.name}`);
  ...
  async function listenForActions() {
    console.log(`\u{1F4E1} Listening for remote actions on Terminal ${terminalId}...`);
    supabase.channel(`terminal-actions-${terminalId}`).on(
      "postgres_changes",
      {
        event: "INSERT",
        schema: "public",
  ...
  async function handleWriteAction(action) {
    ...
    try {
      ...
      try {
        await activeReader.authenticate(4, 96, "FFFFFFFFFFFF");
        console.log(`\u2705 Authentication successful`);
      } catch (authErr) {
        console.log(`\u26A0\uFE0F Auth skipped (Ultralight/NTAG): ${authErr.message}`);
      }
      console.log(`\u23F3 Writing signature to block 4...`);
      const data = Buffer.from(signatureHex, "hex");
      await activeReader.write(4, data, 16);
      console.log(`\u2705 [WRITE SUCCESS] Signature written successfully.`);
      await supabase.from("terminal_actions").update({ status: "COMPLETED", completed_at: (/* @__PURE__ */ new Date()).toISOString() }).eq("id", id);
      try {
